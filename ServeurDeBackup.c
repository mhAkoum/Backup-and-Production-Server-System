...
void handle_client(int client_socket) {
    ...
    while (1) {
        ...
        if (strcmp(buffer, "send") == 0) {
            ...
            read(client_socket, file_content, BUFFER_SIZE);
            printf("File received: %s\n", file_content);

        } else if (strcmp(buffer, "list") == 0) {
            DIR *d;
            struct dirent *dir;
            d = opendir("BackupData");
            if (d) {
...
void statistiques_errorReport(char *erreur) {
    ...
    FILE *file = fopen("BackupStat.txt", "a");
...
int main() {
    ...
    if ((server_fd = socket(AF_INET, SOCK_STREAM, 0)) == 0) {
        statistiques_errorReport("socket failed");
        exit(EXIT_FAILURE);
    }
    ...
    if (listen(server_fd, 3) < 0) {
        statistiques_errorReport("listen failed");
        exit(EXIT_FAILURE);
    }

    printf("Backup Server is running...\n");
